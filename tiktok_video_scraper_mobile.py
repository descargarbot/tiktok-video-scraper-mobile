...
class TikTokVideoScraperMobile:
    ...
    def get_video_data_by_video_id(self, video_id: str) -> tuple:
        ...
        while running:
            if iid_did:
                try:
                    json_video_data = self.tiktok_session.get(tiktok_video_data_endpoint, params=params, headers=self.headers, proxies=self.proxies, timeout=60).json()
                    running = False
                except Exception as e:
        ...
        try:
            tiktok_video_url = json_video_data['aweme_list'][0]['video']['bit_rate'][0]['play_addr']['url_list'][0]
            thumbnail = json_video_data['aweme_list'][0]['video']['dynamic_cover']['url_list'][0]
        except Exception as e:
            print(e, "\nError on line {}".format(sys.exc_info()[-1].tb_lineno))
